...
def _check_viz_deps():
    ...
    if missing:
        raise ImportError(
            ...
            f"Or:            pip install \"datalineageml[viz]\"\n\n"
            f"Active Python: {sys.executable}"
        )


class LineageGraph:
    def __init__(self, store: LineageStore = None): # type: ignore
    ...
    def show(self, output_html: Optional[str] = None):
        ...
        hover_text = [
            ...
            f"hash: {str(G.nodes[n].get('output_hash', ''))[:12]}..."
            for n in G.nodes()
        ]
        ...
        fig = go.Figure(data=[
            ...
            go.Scatter(x=node_x, y=node_y, mode="markers+text",
                       ...
                       text=node_text, hovertext=hover_text,
                       hoverinfo="text", textposition="top center",
                       textfont=dict(size=11)),
        ])
        fig.update_layout(
            title="DataLineageML — Pipeline Lineage Graph",
